...
import { GetBoardDataPathMcpTool } from './getImpactMappingContext/getBoardDataPath.mcpTool.js';
...
import { SubmitScenariosHttpRepository } from './submitScenarios/submitScenarios.httpRepository.js';
...
export class GenerateScenariosModule {
  constructor(server: McpServer, connection: PluginConnection) {
    if (connection.isProxy()) {
      ...
      new GetBoardDataPathMcpTool(server);
      new GetScenariosRequestMcpTool(server, new GetScenariosRequestHttpRepository(port));
      new SubmitScenariosMcpTool(server, connection, new SubmitScenariosHttpRepository(port));
      return;
    }
